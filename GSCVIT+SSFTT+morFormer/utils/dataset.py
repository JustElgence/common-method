...
def load_mat_hsi(dataset_name, dataset_dir):
    ...
    if (dataset_name == 'Salinas'):
        ...
        img = io.loadmat(os.path.join(dataset_dir, dataset_name, 'Houston.mat'))
        image = img['Houston']
        rgb_bands = (59, 40, 23)  # AVIRIS sensor
        gt = io.loadmat(os.path.join(dataset_dir, dataset_name, 'Houston_gt.mat'))['houston_gt']
    ...
    elif (dataset_name == 'fog'):
        image = io.loadmat(os.path.join(dataset_dir, "add_fog_dataset.mat"))
        image = image['Iw']
        gt = io.loadmat(os.path.join(dataset_dir, "PaviaU", "PaviaU_gt.mat"))
        gt = gt['paviaU_gt']
        labels = [
            "Undefined",
            "Asphalt",
    ...
    if (dataset_name == 'IndianPines'):
        ...
        gt = io.loadmat(os.path.join(dataset_dir, dataset_name, "Indian_pines.mat"))
...
def sample_gt_num(gt, percentage, seed = None):

    train_gt = np.full_like(gt, fill_value=-1)
    test_gt = np.full_like(gt, fill_value=-1)

    train_indices, test_indices = [], []
    train_size = np.ones(16)*percentage
    ...
    for c in np.unique(gt):
        ...
        train, test = sklearn.model_selection.train_test_split(X, train_size=int(train_size[c]), random_state=seed)
        train_indices += train
        test_indices += test
...
class HSIDataset1(torch.utils.data.Dataset):
    def __init__(self, image, gt, patch_size, data_aug=True):
        """
        :param image: 3d float np array of HSI, image
        :param gt: train_gt or val_gt or test_gt
