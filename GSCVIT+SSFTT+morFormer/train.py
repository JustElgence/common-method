...
def test1(network, model_dir, image, patch_size, n_classes, device, test_loader):
    network.load_state_dict(torch.load(model_dir + "/model_best.pth"))
    network.eval()
    ...
    for batch_idx, (images, targets, x, y) in enumerate(test_loader):
        ...
        for output, target, x_pos, y_pos in zip(outputs, targets, x, y):

            num_correct = num_correct + (output.item() == target.item())
            total_num = total_num + 1

            matrix[x_pos.item(), y_pos.item()] = output.item()

            matrix1[x_pos.item(), y_pos.item()] = output.item() + 1

    overall_acc = num_correct / total_num

    return overall_acc, matrix, matrix1
