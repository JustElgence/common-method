...
# 汇总训练数据和测试数据
def train_and_test_data(pca_image, band, train_point, test_point, true_point, patch, w, h, n_gcn, device):
    ...
    indexs_train = torch.zeros((train_point.shape[0], n_gcn),dtype=int).to(device)
    indexs_test = torch.zeros((test_point.shape[0], n_gcn),dtype=int).to(device)
    indexs_ture = torch.zeros((true_point.shape[0], n_gcn),dtype=int).to(device)
...
#-------------------------------------------------------------------------------
# train model
def train_epoch(gcn_net,tr_net, train_loader, criterion, optimizer,optimizer2,indexs_train, device):
    objs = AvgrageMeter()
    top1 = AvgrageMeter()
    tar = np.array([])
    pre = np.array([])
...
#-------------------------------------------------------------------------------
# validate model
def valid_epoch(gcn_net,tr_net, valid_loader, criterion,indexs_test, device):
    objs = AvgrageMeter()
    top1 = AvgrageMeter()
    tar = np.array([])
    pre = np.array([])
...
def valid_epoch_draw(gcn_net,tr_net, valid_loader, criterion,indexs_test, device, input):
    m, n = input.shape[:2]
    matrix = np.zeros((m, n))
    objs = AvgrageMeter()
    top1 = AvgrageMeter()
    tar = np.array([])
    pre = np.array([])
    for batch_idx, (A,batch_data, batch_target, indices) in enumerate(valid_loader):
        batch_A = A.to(device)
        batch_data = batch_data.to(device)
        batch_target = batch_target.to(device)
        ...
        tar = np.append(tar, t.data.cpu().numpy())
        pre = np.append(pre, p.data.cpu().numpy())
        for index, (pred, coor) in enumerate(zip(p.data.cpu().numpy(), indices)):
            matrix[coor[0], coor[1]] = pred + 1
        
    return tar, pre, matrix
...
def GET_A2(temp_image,input2,corner,patches,l,device,sigma=10):
...
def load_dataset(args):
    ...
    if Dataset == 'houston2013':
        ...
        data_hsi = img['Houston']
        gt_hsi = sio.loadmat('/home/t1/DL_Code/DATASET/houston2013/' + 'Houston_gt.mat')['houston_gt']
        TOTAL_SIZE = 17270
        TRAIN_SPLIT = args.train_split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * TRAIN_SPLIT)
    
    if Dataset == 'PaviaU':
        uPavia = sio.loadmat('/home/t1/DL_Code/DATASET/PaviaU/' + 'PaviaU.mat')
        gt_uPavia = sio.loadmat('/home/t1/DL_Code/DATASET/PaviaU/' + 'PaviaU_gt.mat')
        data_hsi = uPavia['paviaU']
        gt_hsi = gt_uPavia['paviaU_gt']
        data_hsi = sio.loadmat('/home/t1/DL_Code/DATASET/add_fog_dataset.mat')['Iw']
        TOTAL_SIZE = 42776
        TRAIN_SPLIT = args.train_split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * TRAIN_SPLIT)

    if Dataset == 'PaviaC':
        uPavia = sio.loadmat('/home/t1/DL_Code/DATASET/PaviaC/' + 'Pavia.mat')
        gt_uPavia = sio.loadmat('/home/t1/DL_Code/DATASET/PaviaC/' + 'Pavia_gt.mat')
    ...
    if Dataset == 'Salinas':
        SV = sio.loadmat('/home/t1/DL_Code/DATASET/Salinas/' + 'Salinas_corrected.mat')
        gt_SV = sio.loadmat('/home/t1/DL_Code/DATASET/Salinas/' + 'Salinas_gt.mat')
    ...
    if Dataset == 'IndianPines':
        SV = sio.loadmat('/home/t1/DL_Code/DATASET/IndianPines/' + 'Indian_pines_corrected.mat')
        gt_SV = sio.loadmat('/home/t1/DL_Code/DATASET/IndianPines/' + 'Indian_pines.mat')
        data_hsi = SV['indian_pines_corrected']
        gt_hsi = gt_SV['indian_pines_gt']
        TOTAL_SIZE = 10249
        TRAIN_SPLIT = args.train_split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * TRAIN_SPLIT)
...
def get_data_draw(args):
    ...
    y_test = get_label(test_indices, gt_hsi)-1
    y_true = get_label(total_indices, gt_hsi)-1
    return  data_hsi,CLASSES_NUM,train_indices,test_indices,total_indices,y_train, y_test, y_true, test_indices
