...
parser.add_argument('--dataset', choices=['IndianPines', 'PaviaU', 'Salinas', "PaviaC", "houston2013"],
...
for i in range(0, args.num_run):
    input, num_classes, total_pos_train, total_pos_test, total_pos_true, y_train, y_test, y_true = get_data(args)
    ...
    for epoch in range(args.epoches):
        ...
        if (epoch % args.test_freq == 0) | (epoch == args.epoches - 1)and epoch>=args.epoches*0.6:
            ...
            gcn_net.eval()
            tr_net.eval()
            tar_v, pre_v = valid_epoch(gcn_net, tr_net, label_test_loader, criterion, indexs_test, device)
