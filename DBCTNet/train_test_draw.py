...
from utils.SplitDataset import split_dataset, split_dataset_bynum, split_dataset_draw
...
def test(device, model, test_loader, gt, batch_size):
    m, n = gt.shape[:2]
    ...
    for inputs, labels, indices in test_loader:
        ...
        for i, (pred, index) in enumerate(zip(outputs, indices)):
            x_pos = index // n
            y_pos = index - x_pos * n
            matrix[x_pos, y_pos] = pred + 1


    return y_pred_test, y_test, matrix
...
class to_dataset_draw(torch.utils.data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        return self.data[index],self.gt[index],self.indices[index]
...
def run():
    ...
    x_train,y_train,x_val,y_val,x_test,y_test,sampler,indices = split_dataset_draw(all_data,all_gt,train_ratio=train_ratio)
    train_ds = to_dataset(x_train,y_train)
    val_ds = to_dataset(x_val,y_val)
    test_ds = to_dataset_draw(x_test,y_test,indices)
    ...
    model = model.to(device)
    toc1 = time.time()
    p_t,t_t,matrix=test(device,model,test_loader,gt,batch_size)
    toc2 = time.time()
    display_predictions(convert_to_color(matrix, classes + 1, palette = None), viz, caption="prediction")
